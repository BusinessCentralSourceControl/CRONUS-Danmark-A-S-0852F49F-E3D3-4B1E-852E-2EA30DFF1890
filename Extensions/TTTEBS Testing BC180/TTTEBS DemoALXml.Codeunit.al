codeunit 90001 "TTTEBS DemoALXml"
{
    var // XmlDataLibrary: Codeunit "TTTEBS XmlDataLibraryPTE";
    procedure ReadBooks(xml: Text;
    var Book: Record "TTTEBS BookPTE")var XmlDoc: XmlDocument;
    ...
    begin
        Book.DeleteAll();
        if not XmlDocument.ReadFrom(xml, XmlDoc)then Error('Invalid xml');
        if not XmlDoc.GetRoot(RootElement)then Error('Xml is empty');
    ...
    local procedure ReadBook(BookNode: XmlNode;
    var Book: Record "TTTEBS BookPTE")var Node: XmlNode;
    Attribute: XmlAttribute;
    begin
        if BookNode.AsXmlElement().Attributes().Get('ISBN', Attribute)then Book.ISBN:=CopyStr(Attribute.Value, 1, MaxStrLen(Book.ISBN))
        else
            Book.ISBN:='X-XXXXXX-XX-X';
        BookNode.SelectSingleNode('title', Node);
        Book.Title:=CopyStr(Node.AsXmlElement().InnerText, 1, MaxStrLen(Book.Title));
        if BookNode.SelectSingleNode('author/first-name', Node)then begin
            Book.Author:=CopyStr(Node.AsXmlElement().InnerText, 1, MaxStrLen(Book.Author));
...
// end;
// local procedure ReadBookWithNamespace(BookNode: XmlNode; XmlNamespaceMgr: XmlNamespaceManager; var Book: Record "TTTEBS BookPTE")
// var
//     Node: XmlNode;
//     Attribute: XmlAttribute;
// begin
